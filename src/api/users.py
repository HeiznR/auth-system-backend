...
@router.get("/users/{id}", tags=["users"], summary="get user by id")
async def get_user_by_id(id: str) -> UsersSchema:
    try:
        user = await UsersService(UsersRepository()).read_user_by_id(id)
        return user
    except Exception as e:
        print(f"an error occured: {e}")
